...
def check_auth(username, password):
    return username == "admin" and password == "finanskobi2026"

def authenticate():
    return Response(
        "Giris yapmaniz gerekiyor.", 401,
        {"WWW-Authenticate": 'Basic realm="FinansKobi"'}
    )
...
for endpoint in list(app.server.view_functions):
    app.server.view_functions[endpoint] = requires_auth(
        app.server.view_functions[endpoint]
    )
